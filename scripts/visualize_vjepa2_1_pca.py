...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--checkpoint", type=Path, default=None, help="Pretrained or fine-tuned checkpoint. Defaults to meta.read_checkpoint from --config.")
    parser.add_argument(
        "--checkpoint-key",
        default="auto",
    ...
    parser.add_argument("--num-frames", type=int, default=None, help="Frames sampled from each video. Defaults to config data.dataset_fpcs[0].")
    parser.add_argument("--frame-indices", default=None, help="Comma-separated video frame indices. Overrides uniform sampling.")
    ...
    parser.add_argument("--pca-scope", choices=["batch", "sample"], default="batch", help="Fit PCA once for all samples or separately per sample.")
    parser.add_argument("--max-pca-tokens", type=int, default=50000, help="Subsample tokens used to fit PCA; all tokens are projected.")
...
def build_encoder(cfg: dict, args: argparse.Namespace, device: torch.device) -> torch.nn.Module:
    data_cfg = cfg.get("data", {})
    model_cfg = cfg.get("model", {})
    model_name = args.model_name or model_cfg.get("model_name", "vit_base")
...
def main() -> None:
    ...
    for sample in samples:
        ...
        all_tokens.append(feature_map.reshape(-1, feature_map.shape[-1]))

    batch_mean = batch_components = batch_lo = batch_hi = None
    if args.pca_scope == "batch":
        batch_mean, batch_components = fit_pca(torch.cat(all_tokens, dim=0), args.max_pca_tokens)
        batch_projected = (torch.cat(all_tokens, dim=0) - batch_mean) @ batch_components
        batch_lo, batch_hi = pca_limits(batch_projected)

    for sample, raw_frames, feature_map in prepared:
        flat = feature_map.reshape(-1, feature_map.shape[-1])
        if args.pca_scope == "sample":
            mean, components = fit_pca(flat, args.max_pca_tokens)
            lo = hi = None
        else:
            mean, components = batch_mean, batch_components
            lo, hi = batch_lo, batch_hi
        pca_map = project_pca(flat, mean, components, lo, hi).view(*feature_map.shape[:3], 3)
